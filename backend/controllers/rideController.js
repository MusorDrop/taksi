...
// Общая часть SELECT для поездок (используется в getRides и getMyRides)
const RIDE_SELECT = `
        SELECT 
            r.id,
            r.driver_id,
...
async function createRide(req, res) {
    ...
    // Валидация координат: если передана только часть — возвращаем 400 вместо
    // тихой подмены маршрута на дефолтный (Уралмаш → Новокольцовский)
    const startCheck = validateCoordinates(
        req.body.start_lat,
        req.body.start_lon,
        'точки отправления (start)'
    );
    if (startCheck.error) {
        return res.status(400).json({ error: startCheck.error });
    }
    const endCheck = validateCoordinates(
        req.body.end_lat,
        req.body.end_lon,
        'точки назначения (end)'
    );
    if (endCheck.error) {
    ...
    const startCoords = resolvePointCoordinates(
        req.body.start_point || req.body.from || startCheck.point,
        DEFAULT_START
    );
    const endCoords = resolvePointCoordinates(
        req.body.end_point || req.body.to || endCheck.point,
        DEFAULT_END
    );

    const departureTime = parseDepartureTime(req.body.departure_time || req.body.time);

    // Поездка с временем отправления в прошлом никогда не попадёт в выдачу поиска
    // (getRides фильтрует r.departure_time > NOW()) и станет «невидимой» для пассажиров.
    // Отклоняем такой запрос с понятной ошибкой вместо тихого создания пустышки.
    if (departureTime.getTime() <= Date.now() - 60 * 1000) {
        return res.status(400).json({
            error: 'Время отправления не может быть в прошлом. Укажите будущие дату и время выезда.'
        });
    }

    // Валидация количества мест (защита от NaN → ошибка БД/500)
    let totalSeats = 4;
    ...
    if (req.body.available_seats !== undefined && req.body.available_seats !== null && String(req.body.available_seats).trim() !== '') {
        availableSeats = parseInt(req.body.available_seats, 10);
        if (isNaN(availableSeats) || availableSeats < 0) {
            return res.status(400).json({
                error: 'Параметр available_seats должен быть неотрицательным целым числом'
            });
        }
    }

    availableSeats = Math.min(totalSeats, availableSeats);
...
async function getRides(req, res) {
    ...
    // Только поездки, где ещё есть свободные места
    const conditions = ["r.status = 'scheduled'", 'r.available_seats > 0'];
    const params = [];
...
async function getMyRides(req, res) {
    ...
    try {
        ...
        const passengerRes = await pool.query(
            `${RIDE_SELECT}
            INNER JOIN matches m ON m.ride_id = r.id
            WHERE m.passenger_id = $1 AND m.status = 'accepted'
            ORDER BY r.departure_time ASC`,
            [userId]
        );

        return res.json({
            driver_rides: driverRes.rows.map(mapRideRow),
            passenger_rides: passengerRes.rows.map(mapRideRow)
        });
    } catch (err) {
        console.error('Ошибка получения поездок пользователя:', err);
        return res.status(500).json({ error: 'Внутренняя ошибка сервера при получении поездок пользователя' });
    }
}
