...
- `GET /api/rides` — список доступных поездок со свободными местами с поддержкой гео-фильтрации и фильтра по времени (публичный):
  ...
  - **Возвращаемые данные:** список поездок с полями `distance_km`, `is_peak`, `base_price`, информацией о водителе (имя, рейтинг, телефон) и привязанном автомобиле (`vehicle_id`).
